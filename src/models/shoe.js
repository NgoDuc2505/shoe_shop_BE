...
export const getShoeList = async (req, res) => {
    try {
        const shoes_list = await db.collection(process.env.DB_SHOE_COLLECTION).find().toArray();
...
export const updateShoe = async (req, res) => {
    try{
        ...
        const result = await db.collection(process.env.DB_SHOE_COLLECTION).updateOne(
        ...
        console.log(error);
        return res.status(500).json({ error: "Error updating shoe [Invalid parameter]", errorLog: error });
    }
}
...
export const findShoeByID = async (req, res) => {
    try{
        const shoeId = req.params.id;
        const result = await db.collection(process.env.DB_SHOE_COLLECTION).findOne({ "_id": new ObjectId(shoeId) });
        if (result) {
            return result;
        } else {
            return null;
        ...
        return res.status(500).json({ error: "Error finding shoe by ID [Invalid parameter]" });
    }
}
...
export const deleteShoe = async (req, res) => {
    try{
        ...
        return res.status(500).json({ error: "Error deleting shoe [Invalid parameter]" });
    }
}

export const getDeletedShoe = async (req, res) => {
    try{
        const shoes_list = await db.collection(process.env.DB_SHOE_COLLECTION).find({isDel: true}).toArray();
        return shoes_list
    }catch(error){
