...
import { addNewShoe, getShoeList, updateShoe, findShoeByID, deleteShoe, getDeletedShoe} from "../models/shoe.js";
...
export const handleGetShoesByName = async (req, res) => {
    try{
        ...
        if(name) {
            const shoes = await getShoeList(req, res);
            const shoe_name = shoes.filter(shoe => shoe.name.toLowerCase().includes(name.toLowerCase()));
...
export const handleGetShoeCategory = async (req, res) => {
    try{
        ...
        successResponse(res, category_list, "Shoe category list fetched successfully");
    }catch(error){
...
export const handleGetShoeByCategory = async (req, res) => {
    try{
        ...
        if(category) {
            ...
            successResponse(res, shoe_category, "Shoe category list fetched successfully");
        }else{
